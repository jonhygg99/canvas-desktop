...
mod tests {
    ...
    /// hilo sirve una sola petición. Antes de responder drena el request del
    /// cliente (las cabeceras hasta `\r\n\r\n`) y cierra SOLO con el `drop`
    /// — sin `shutdown(Both)`.
    ///
    /// Por qué: si el servidor cierra sin haber leído lo que el cliente
    /// envió, macOS responde al `close` con RST en vez de FIN, y la
    /// limpieza del pool de conexiones de ureq (`return_to_pool` →
    /// `set_read_timeout(None)`) choca con ese socket a medio cerrar con
    /// `EINVAL` y panica («returning stream to pool: Os { code: 22 }»). Eso
    /// hacía flaky estos tests bajo carga. Drenando el request el cierre es
    /// FIN limpio y el pool de ureq no disputa.
    fn serve_response(status: &str, body: &[u8]) -> String {
        ...
        thread::spawn(move || {
            ...
            // Drena las cabeceras del request para que el cierre sea
            // FIN limpio (sin datos sin leer → nunca RST). El GET de ureq
            // llega completo de golpe, así que este bucle termina rápido.
            let mut req = Vec::new();
